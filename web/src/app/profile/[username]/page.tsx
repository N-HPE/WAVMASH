...
import type { UserProfile, Track, Post } from '@/lib/types';
  ...
  useEffect(() => {
    async function loadProfileData() {
      ...
      try {
        ...
        const [tracksRes, postsRes, likesRes] = await Promise.allSettled([
          ...
          <div className="flex-1 text-center sm:text-left space-y-3 min-w-0">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              {isMyProfile && (
                <Link
                  href="/settings"
                  className="p-2 rounded-xl bg-white/5 hover:bg-white/10 text-white/70 hover:text-white transition-colors border border-white/10 self-center sm:self-auto"
                  aria-label="설정"
                  title="설정"
                >
